...
| Tushare | `daily_basic`, `moneyflow`, `margin_detail`, `block_trade`, `top_list` | A 股 EOD 行情、资金流、两融、大宗、龙虎榜 | 交易日 15:05-18:30 CST | 5m 至有当日分区；之后 30m | 现在 collector 每 60 秒重拉，但这些值通常只在盘后更新。建议拆出 EOD 调度，避免把 EOD 接口伪装成分钟行情。`top_list` 是代码实调用，但不在当前 `tushare_endpoints.csv` 可用清单中，需后续补 catalog。 |
...
| FMP | `/income-statement`, `/balance-sheet-statement`, `/cash-flow-statement`, `/ratios-ttm` | 美股财报 / TTM 估值 | 财报季 05:00-09:30、16:00-22:00 ET；非财报季 06:30、17:30 ET | 财报季 30m；非财报季 daily | 代码使用 `/stable/*`；`/ratios-ttm` 是代码实调用，CSV 里用其他 FMP 文档口径表示 TTM/ratio 能力。 |
...
按 `docs/data_sources/tushare_endpoints.csv` 中 `status=covered` 统计，可用接口 99 条；其中 Raw Zone adapter 已有 typed asset 的 41 个接口应优先按本表落地。`trade_cal` 在 CSV 中有股票/期货两行，调度上按同一接口处理。
...
| A_CORPORATE_EVENTS | `anns`, `dividend`, `repurchase`, `share_float`, `stk_holdernumber`, `pledge_detail`, `pledge_stat`, `stk_holdertrade`, `stk_surv`, `forecast`, `express`, `disclosure_date`, `new_share`, `fina_mainbz`, `report_rc` | 公告、分红回购、解禁、股东、质押、调研、业绩预告/快报、主营构成、券商预测 | 交易日 07:00-09:30、11:30-13:00、15:00-23:30 CST；财报季扩大到每日 | 财报/公告季 15m；平时 60m + daily |
...
| A_CROSS_BORDER_FLOW | `hk_hold`, `hsgt_top10`, `ggt_daily`, `ggt_monthly`, `moneyflow_hsgt`, `stk_ah_comparison` | 沪深港通、港股通、AH 溢价 | 交易日 16:30-20:30 CST；月度数据月初 06:30 补扫 | daily 15m 到出现；monthly daily until present |
...
按 `docs/data_sources/fmp_endpoints.csv` 当前 `currently_subscribed=yes` 统计，Starter 当前可直接调用接口 42 条。Premium/Ultimate 锁定接口不应进入当前生产调度，除非订阅字段更新。
...
| US_FUNDAMENTALS | `/income-statement`, `/balance-sheet-statement`, `/cash-flow-statement`, `/key-metrics`, `/key-metrics-ttm`, `/ratios`, `/financial-growth`, `/enterprise-values` | 财报、指标、估值 | 财报季 05:00-09:30、16:00-22:00 ET；非财报季 06:30、17:30 ET | 财报季 30m；非财报季 daily |
...
| US_INDEX_MACRO | `/historical-index`, `/sector-performance`, `/stock-market-performance`, `/treasury`, `/economic` | 指数、板块表现、国债利率、经济指标 | 指数随 US_LIVE_QUOTES；宏观/treasury 在 07:00-18:00 ET | 指数 1m/15m；宏观发布日 15m，否则 daily |
...
| US_FILINGS_TRANSCRIPTS_PREMIUM | `/sec_filings`, `/rss_feed`, `/earning_call_transcript`, `/earning_call_transcript-list` | Premium | 06:00-23:00 ET；财报季扩大 | 15m |
| US_ANALYST_13F_PREMIUM | `/analyst-estimates`, `/analyst-stock-recommendations`, `/price-target`, `/upgrades-downgrades`, `/grade`, `/institutional-holder`, `/13F`, `/etf-holder`, `/institutional-symbol-ownership` | Premium | 06:00-22:00 ET | 60m；13F 窗口 daily |
| US_DCF_QUALITY_PREMIUM | `/discounted-cash-flow`, `/levered-discounted-cash-flow`, `/historical-discounted-cash-flow-statement`, `/financial-score`, `/owner_earnings`, `/market_risk_premium`, `/mergers-acquisitions`, `/executive-compensation`, `/insider-roster`, `/insider-trades-statistics` | Premium | 06:30、17:30 ET | daily |
...
按 `docs/data_sources/futu_endpoints.csv` 中 `covered` + `operational` 统计，当前可用/运维接口 34 条。实际权限仍以 `get_global_state`、`query_subscription` 和每次 SDK 返回码为准。
...
6. 复核发现两个 catalog/code 口径差异：Tushare `top_list` 和 FMP `/ratios-ttm` 是当前代码实调用，但不在对应 CSV 的当前精确接口名中；后续若把本文档转为机器调度配置，应先补齐 catalog。
