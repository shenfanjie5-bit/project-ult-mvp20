...
def test_current_mvp_score_applicability_excludes_review_gated_fields(tmp_path: Path) -> None:
    ...
    score_sink_effect_path = tmp_path / "score_sink.json"
    ...
    _write_json(score_sink_effect_path, {"summary": {}, "rows": []})
...
def test_current_mvp_score_applicability_excludes_verified_no_effect_score_sink(
    tmp_path: Path,
) -> None:
    field_closure_path = tmp_path / "field_closure.json"
    conversion_path = tmp_path / "conversion.json"
    ...
    _write_json(execution_path, {"summary": {"execution_status": "executed"}})
    _write_json(
        score_sink_effect_path,
        {
            "summary": {"no_final_score_delta_count": 1},
            "rows": [
                {
                    "dp_id": "L10.val.peer",
                    "score_target": "confidence_multiplier",
                    "max_abs_delta": 0.0,
                    "no_final_score_delta": True,
                }
            ],
        },
    )
        ...
        yaml.safe_dump(
            {
                "exclusion_policies": {
                    "score_sink_no_effect_verified": {
                        "mvp_applicability": "excluded_from_current_mvp_denominator",
                        "applies_when": {"dp_id_in": ["L10.val.peer"]},
                    }
                }
            }
        ),
    ...
    summary = report["summary"]
    assert summary["raw_score_relevant_final_target_count"] == 1
    assert summary["raw_current_numeric_final_score_count"] == 1
    assert summary["current_mvp_denominator_count"] == 0
    assert summary["current_mvp_excluded_count"] == 1
    assert summary["excluded_reason_counts"] == {"score_sink_no_effect_verified": 1}
    assert report["excluded_rows"][0]["score_sink_no_final_score_delta"] is True
